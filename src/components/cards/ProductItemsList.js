...
const ProductItemsList = ({product}) => {
	...
	return(
		<ul className='list-group'>
			...
			<li className='list-group-item'>
				Category <Link to={category&&`category/${category.slug}`} className='label label-default label-pill pull-xs-right'>{category&&category.name}</Link>
			</li>
			{subs&&(
				<li className='list-group-item'>
					Sub Category 
					{subs.map((sub)=>(
						<Link to={`sub/${sub.slug}`} key={sub._id} className='label label-default label-pill pull-xs-right'>
							{sub.name}
						</Link>))
					}
				</li>)
			...
			<li className='list-group-item'>
				Avalible <span className='label label-default label-pill pull-xs-right'>{quantity}</span>
			</li>
			<li className='list-group-item'>
